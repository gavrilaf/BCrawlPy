...
class Runner(MQ.BaseProducer):
	...
	def process(self, p, out_queue):
		provider = Providers.content_provider_by_host(p.host)
		if provider == -1:
			self.logger.error('Post (%s, %s). Unknown provider' % (p.link, p.host))
			raise Exception('Unknown provider')

		if provider not in self._content_readers:
			...
			try 
				reader = self._content_readers[provider]
	...
	def on_start(self, connection):
		...
		self._readers = [
			Providers.LJ, LJ.ContentReader(self.name, self._monitor),
			Providers.VK, VK.ContentReader(self.name, self._monitor),
		]
